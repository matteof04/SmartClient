...
class DeviceChangeUpdFreq: Command {
    private val smartApi: SmartApi by KoinJavaComponent.getKoin().inject()

    override fun getCommandString() = "CHANGE_UPD_FREQ"

    override fun getHelpString() = """
        Usage: CHANGE_UPD_FREQ <deviceId> <newUpdFreq>
        Change the update frequency for the device with the specified deviceId. The newUpdFreq is expressed in milliseconds
    """.trimIndent()

    override fun execute(handler: CommandHandler, args: List<String>) {
        try {
            val deviceId = UUID.fromString(args[0])
            val newUpdFreq = args[1].toUInt()
            runBlocking {
                smartApi.changeUpdateFrequency(deviceId, newUpdFreq)
                println("Success")
            }
