...
class DeviceBeginAssoc: Command {
    private val smartApi: SmartApi by KoinJavaComponent.getKoin().inject()

    override fun getCommandString() = "BEGIN"

    override fun getHelpString() = """
        Usage: BEGIN <deviceId>
        Start the association process for the device with the given device ID
    """.trimIndent()
    ...
    override fun execute(handler: CommandHandler, args: List<String>) {
        try {
            ...
            runBlocking {
                smartApi.beginDeviceAssoc(deviceId)
            }
            println("Success")
