...
class DeviceAssocSuperCommand: SuperCommand() {
    init {
        subCommandHandler.registerCommand(DeviceBeginAssoc())
        subCommandHandler.registerCommand(DeviceResetAssoc())
        subCommandHandler.registerCommand(DeviceHouseAssoc())
    }

    override fun getCommandString() = "ASSOC"

    override fun getHelpString() = """
        Usage: ASSOC <subcommand>
        Device-related association commands
    """.trimIndent()
}