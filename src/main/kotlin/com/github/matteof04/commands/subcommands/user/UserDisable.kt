...
class UserDisable: Command {
    private val smartApi: SmartApi by KoinJavaComponent.getKoin().inject()

    override fun getCommandString() = "DISABLE"
    override fun getHelpString() = """
        Usage: DISABLE <userId>
        Disable the user specified by the userId
        ADMIN ONLY COMMAND! 
    """.trimIndent()
    ...
    override fun execute(handler: CommandHandler, args: List<String>) {
        try {
            runBlocking {
                val userId = UUID.fromString(args.first())
                smartApi.disableUser(userId)
                println("Disabled")
            }
