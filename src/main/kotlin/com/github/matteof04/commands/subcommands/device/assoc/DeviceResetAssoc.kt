...
class DeviceResetAssoc: Command {
    ...
    override fun getHelpString() = """
        ...
        Reset the association for the device with the given device ID
    """.trimIndent()
    ...
    override fun execute(handler: CommandHandler, args: List<String>) {
        try {
            ...
            println("""
                |<=== WARNING! ===>
                |If you continue, the device will be removed from your account.
                |To continue, type "Yes, i want to reset my device" after this message""".trimMargin())
            if (readlnOrNull() == "Yes, i want to reset my device") {
                runBlocking {
                    smartApi.resetDeviceAssoc(deviceId)
                }
                println("Done")
            }else{
