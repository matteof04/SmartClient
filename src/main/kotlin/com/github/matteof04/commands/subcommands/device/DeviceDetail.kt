...
class DeviceDetail: Command {
    ...
    override fun getHelpString() = """
        Usage: DETAIL <deviceId>
        Retrieve details about the device with the specified deviceId
    """.trimIndent()
    ...
    override fun execute(handler: CommandHandler, args: List<String>) {
        try {
            ...
            runBlocking {
                val device = smartApi.getDeviceDetail(deviceId)
                println("""
                    ID: ${device.id}
                    #    Type: ${device.type}
                    #    Update Frequency: ${device.updateFrequency}
                    #    Host ID: ${device.hostId}
                    #    House ID: ${device.houseId}
                    #    Owner ID: ${device.ownerId}
                    #    Association State: ${device.assocState}
